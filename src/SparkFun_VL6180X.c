...
int newRangeData(void)
{
    ...
    readStatus = VL6180x_getRegister(VL6180X_RESULT_RANGE_STATUS);
...
{
  uint8_t data;
  I2Cdev_readByte2(_i2caddress, registerAddr, 1, &data);
  return data;
}
...
{
  uint16_t data;
  I2Cdev_readByte2(_i2caddress, registerAddr, 2, &data);
  return data;
}

void VL6180x_setRegister(uint16_t registerAddr, uint8_t data)
{
  I2Cdev_writeByte2(_i2caddress, registerAddr, 1, &data);
  return;
}

void VL6180x_setRegister16bit(uint16_t registerAddr, uint16_t data)
{
    uint8_t twoDataBytes[2];
    twoDataBytes[0] = data >> 8;
    twoDataBytes[1] = data && 0xFF;
    I2Cdev_writeByte2(_i2caddress, registerAddr, 2, twoDataBytes);
    return;
}
